...
template <typename CacheTrait>
void CacheAllocator<CacheTrait>::initEC(EvictionController<CacheTrait>* ec, size_t mm_size) {
  ...
  if (mm_size < 1024) {
    ...
    ec->threshold = ec->logical_time * 10;
...
template <typename CacheTrait>
void CacheAllocator<CacheTrait>::evictByEC(EvictionController<CacheTrait>* ec, Item*& toRecycle_) {
  ...
  if (ec->evict_queue.try_dequeue(evict_pointer)) {
    ...
    XLOG_EVERY_MS(INFO, 1000) << "eq empty: " << ec->candidate_queue_size << ' ' << ec->evict_queue_size
     << " token: " << enqueue_token[ec->cid];
  }
...
void CacheAllocator<CacheTrait>::enqueueCandidateEC(EvictionController<CacheTrait>* ec, MMContainer& mmContainer) {
  ...
  if (ec->candidate_queue_size < enqueue_batch_size && ec->evict_queue_size < enqueue_batch_size) {
    ...
    for (int i = 0; i < enqueue_batch_size; i += ec->training_sample_rate) {
      ...
      if (mmContainer.isLRU()) {
        items_for_prediction[i]->set_item_flag2(1);
      }
      n_reinsertion_queue ++;
    }
...
CacheAllocator<CacheTrait>::getNextCandidate(PoolId pid,
  ...
  //                                  &token](auto&& itr) {
    ...
    if (!itr) {
      ...
      XLOG_EVERY_MS(INFO, 1000) << cid << ' ' << searchTries << ' ' << (*stats_.evictionAttempts)[pid][cid].get();
    }
    ...
    while ((config_.evictionSearchTries == 0 ||
      ...
      if (config_.useEvictionControl) {
        ...
        if (ec->window_size == 0) {
          initEC(ec, mm_size);
          XLOG(INFO) << "Begin eviction in class " << int(cid) << " window size: " << ec->window_size
            << " train batch: " << ec->training_batch_size;
          enqueue_token[cid].store(1);
        }
        
        if (ec->use_eviction_control > 0) {
          evictByEC(ec, toRecycle_);
          if (enqueue_token[cid].exchange(0) == 1) {
            enqueueCandidateEC(ec, mmContainer);
            enqueue_token[cid].store(1);
          }
        }
        itr.resetToBegin();
      ...
      if (toRecycle_ == nullptr) {
        toRecycle_ = itr.get();
        mmContainer.moveToHeadLocked(*toRecycle_);
        //XLOG_EVERY_MS(INFO, 1000) << toRecycle_->isMarkedForEviction() << ' ' << toRecycle_->get_item_flag2();
        //toRecycle_->set_item_flag2(1);
      }
      if (!toRecycle_) {
        XLOG_EVERY_MS(INFO, 1000) << "null";
      ...
      if (evictToNvmCache && !putToken.isValid()) {
        ...
        ++itr;
        XLOG_EVERY_MS(INFO, 1000) << "put token invalid: " << stats_.evictFailConcurrentFill.get();
        continue;
      }
      ...
      if (!markedForEviction) {
        ...
        ++itr;
        // XLOG_EVERY_MS(INFO, 1000) << "markForEviction fail: " << stats_.evictFailAC.get();
        continue;
      }
...
template <typename CacheTrait>
void CacheAllocator<CacheTrait>::render() {
  double interval = std::chrono::duration_cast<std::chrono::nanoseconds>(
    std::chrono::system_clock::now() - timePeriod).count() / 1e9;
...
*/
  ...
  int64_t evicted_total = 0, evicted_heur_total = 0;
  for (ClassId cid = 0; cid < static_cast<ClassId>(allocSizes.size()); ++cid) {
    if (config_.useEvictionControl) {
      EvictionController<CacheTrait>* ec = evictionControllers_[0][cid];
      ec->info();
      evicted_total += ec->examined_cnt - ec->reinserted_cnt;
      evicted_heur_total += ec->n_evict_by_heur;
    }
  }
  ...
  //std::cout << "wall clock time: " << wall_clock_elapse / 1000.0 << '\n';
  std::cout << "*throughput: " << (cacheParsedCnt - prev_cacheParsedCnt) / interval << '\n';
  std::cout << "*target throughput: " << (cacheReqCnt - prev_cacheReqCnt) / interval << '\n';
  prev_cacheReqCnt = cacheReqCnt;
  prev_cacheParsedCnt = cacheParsedCnt;
  */

  n_miss = 0;
  std::cout << "find time: " << find_time / 1e9 << '\n';
  std::cout << "insert time: " << insert_time / 1e9 << '\n';
  std::cout << "  meta update time: " << meta_update_time / 1e9 << '\n';
  std::cout << "allocate time: " << allocate_time / 1e9 << '\n';
  std::cout << "  eviction time: " << eviction_time / 1e9 << '\n';
  std::cout << "    enqueue time: " << enqueue_time / 1e9 << '\n';
  std::cout << "    dequeue time: " << dequeue_time / 1e9 << '\n';
  std::cout << "  release time: " << release_time / 1e9 << '\n';
  find_time = 0;
  release_time = 0;
  eviction_time = 0;
  allocate_time = 0;
  ...
  enqueue_time = 0;
  dequeue_time = 0;
  //std::cout << "prediction time: " << prediction_time / 1e9 << '\n';
  std::cout << "reinsertion queue: " << n_reinsertion_queue << '\n';
  std::cout << "eviciton queue: " << n_eviction_queue << '\n';
  std::cout << "evict queue empty: " << n_evict_empty << '\n';
  n_predict = 0;
  n_eviction_queue = 0;
  n_evict_out_cache = 0;
  n_evict_empty = 0;
  n_reinsertion_queue = 0;
  ...
  if (config_.useEvictionControl) {
    ...
    for (ClassId cid = 0; cid < static_cast<ClassId>(allocSizes.size()); ++cid) {
      ...
      ec->train_data_time = 0;
      ec->update_time = 0;
      ec->build_time = 0;
      ec->predict_time = 0;
    }
    std::cout << "build feature time: " << total_build_time / 1e9 << '\n';
    std::cout << "predict time: " << total_predict_time / 1e9 << '\n';
  }
  std::cout << std::endl;
  ...
  if (debug_mode > 1) {
    ...
    const auto& allocSizes = pool.getAllocSizes();
    for (ClassId cid = 0; cid < static_cast<ClassId>(allocSizes.size()); ++cid) {
      auto& mmContainer_ = getMMContainer(pid, cid);
      EvictionController<CacheTrait>* ec = evictionControllers_[pid][cid];
      auto itr = mmContainer_.getEvictionIterator();
      if (ec->reinserted_cnt > 0 || debug_mode >= 1) {
        std::cout << int(cid) << ' '
          ...
          //<< ", reinsertions: " << ec->reinserted_cnt
          //<< ", empty: " << n_evict_empty
          << ", rpe: " << ec->reinserted_cnt << ',' << ec->examined_cnt - ec->reinserted_cnt
          //<< ", rpe target: " << ec->reinsertion_per_eviction
          << ", cq: " << ec->candidate_queue.size_approx()
          << ", eq: " << ec->evict_queue.size_approx()
          << ", v tta: " << ec->victim_tta / ec->victim_cnt
          << ", r tta: " << ec->reinsert_tta / ec->reinsert_cnt
          //<< ", ec: " << ec->use_eviction_control
          << std::endl;
      }

      n_reinsertion += ec->reinserted_cnt;
      ec->reinserted_cnt = 0;
      ec->examined_cnt = 0;
      ec->victim_tta = 0;
      ec->reinsert_tta = 0;
...
template <typename CacheTrait>
void CacheAllocator<CacheTrait>::createEvictionControllers(const PoolId pid) {
  const auto pools = getPoolIds();
  const auto& pool = allocator_->getPool(pid);
  const auto& allocSizes = pool.getAllocSizes();
  evictionControllers_.push_back(
      std::vector<EvictionController<CacheTrait>*>(allocSizes.size()));
  for (int cid = 0; cid < allocSizes.size(); cid ++) {
    // cout << "created eviction controller for pool and class: ";
    evictionControllers_[pid][cid] = new EvictionController<CacheTrait>(config_.MLConfig);
    evictionControllers_[pid][cid]->cid = cid;
  ...
  if (bfRatio > 0 && pid == 0 && nvmCache_)
    nvmCache_->makeBf(config_.getCacheSize() * bfRatio * 8);
